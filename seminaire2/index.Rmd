...
Il n'existe présentement pas d'outil construit sur une base théorique solide pour comprendre comment les distributions d'espèces spécialistes changeront en réponse au réchauffement climatique
...
Pour explorer ces questions, je présente un modèle et puis une série d'aproximations de ses dynamiques. Je compare les résultats des simulations avec ceux des approximations et j'utilise les approximations pour répondre aux questions.
...
Structure spatiale du paysage (fragmentation)
...
## Effet d'un décalage sur la vitesse de réponse
...
## Effet d'un décalage sur la persistence 
...
Fraction des patches favorables occupées à l'équilibre

$$P_{A}^{*} = 1 - \frac{\delta_{C}}{R}$$

$$P^{*} = R - \delta_{C}$$

???

Un décalage est la modification des taux observés (e/c) de façon à affecter la persistence du consommateur.

Un décalage dans la réponse de la ressource modifie la fraction des patches occupées de façon à ce que le taux de changement du consommateur à l'échelle du paysage soit ralenti et accéléré à l'échelle des patches favorables si la disponibilité d'habitat ne change pas.

---

# Les métapopulations

## Effet de la fragmentation et du paysage
