...
curve(1-((x)/(1-x)), 0,1,100, ylim=c(0,1), 
    ...
    ylab=expression(P^'*'), xlab='Temps (e/c)')
...
# Modèle .small[.grey[\- exemple 3]]

## Perte de connectivité du paysage

La probabilité moyenne des patches de forêt boréale d'être occupées $P_{\lambda}^{*}$ par la grive dépend de ses caractéristiques propres $\frac{e}{c}$ et de la structure spatiale (connectivité) du paysage $\lambda_{M}$
...
plot(x=x, y=y, 
    ...
    cex.lab=2, cex.axis=1.5,
    ylab=expression(P[lambda]^'*'), xlab=expression(lambda[M]))
```
]
...
Capacité de métapopulation: première valuer propre de la matrice de 'paysage'
...
## Modélisation de la réponse de la grive de Bicknell

$H_{0}$: Correlation entre conditions climatiques et présence 

???

La réponse de la grive au réchauffement du climat a été décrite par deux hypothèses/théories:

1. L'hypothèse nulle est représentée par le SDM qui corrèle la distribution d'une espèce à la présence de conditions climatiques spécifiques

--

$H_{1}$: Taux d'expansion en fonction de traits spécifiques

???

2. H_{1} est l'inclusion de traits spécifiques tels que des processus démographiques et des limites de dispersion sur la capacité à suivre le déplacement des conditions climatiques favorables

--

$H_{Victor}$: Trois facteurs de complexité

> Dispersion - climat

> Biotique - abiotique

> Dispersion - fragmentation - taille des parcelles 

???

Mon hypothèse est que la réponse de la grive est déterminée par le 
- déplacement de ses conditions climatiques favorables, 
- Sa capacité de dispersion et démographie
- sa dépendance à l'habitat boréale et la réponse de l'habitat au réchauffement du climat, 
- ainsi qu'à la disposition spatiale du paysage
